...
namespace ggp
{
	namespace ExeOpt
	{
		bool stopCalcExe( QString strExePath )
		{
			...
			while (bFlag != FALSE)
			{
				if (_tcscmp(processInfo.szExeFile, strExePath.toLocal8Bit().data()) == 0) 
				{
        ...
        HANDLE getProcessHandle( char* strExePath )
        {
            ...
            while (bFlag != FALSE)
            {
                if (filePath.compare(processInfo.szExeFile, Qt::CaseInsensitive) == 0) // 之前的方法存在大小写问题
